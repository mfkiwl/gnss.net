...
namespace Asv.Gnss
{
    public static class UbxHelper
    {
        ...
        public static byte[] GenerateRequest(byte @class, byte subclass, byte[] payload = null)
        {
            ...
            data[5] = (byte)((payloadLength >> 8) & 0xff);


            if (payload != null)
                Array.ConstrainedCopy(payload, 0, data, 6, payloadLength);

            var crc = UbxCrc16.CalculateCheckSum(data, data.Length - 2);
            data[data.Length-2] = crc.Crc1;
            data[data.Length-1] = crc.Crc2;
            return data;
        }
