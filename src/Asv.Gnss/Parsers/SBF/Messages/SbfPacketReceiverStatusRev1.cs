...
namespace Asv.Gnss
{
    public class SbfPacketReceiverStatusRev1 : SbfPacketBase
    {
        ...
        protected override void DeserializeMessage(byte[] buffer, uint offsetBits)
        {
            ...
            CPULoad = buffer[offset];offset+=1;
            ExtError = (SbfExtErrorEnum)buffer[offset]; offset += 1;
            UpTime = TimeSpan.FromSeconds(BitConverter.ToUInt32(buffer, offset));offset+=4;
            RxState = (SbfRxStateEnum) BitConverter.ToUInt32(buffer, offset);offset+=4;
            RxError = (SbfRxErrorEnum)BitConverter.ToUInt32(buffer, offset); offset += 4;
            ...
            for (var i = 0; i < N; i++)
            {
                ...
                AGCState[i].Deserialize(buffer, offset, sbLength);
                offset += sbLength;
            }
    ...
    {
        public void Deserialize(byte[] buffer, int offsetInBytes, byte sbLength)
        {
            var FrontEndID = buffer[offsetInBytes]; offsetInBytes+=1;
            FrontEndCode = (SbfFrontEndCodeEnum) (FrontEndID & 0b0001_1111);
            Antenna = (AntennaId) (FrontEndID >> 5);
            Gain = (sbyte)buffer[offsetInBytes]; offsetInBytes+=1;
            SampleVar = buffer[offsetInBytes]; offsetInBytes += 1;
            BlankingStat = buffer[offsetInBytes]; offsetInBytes += 1;
        }
