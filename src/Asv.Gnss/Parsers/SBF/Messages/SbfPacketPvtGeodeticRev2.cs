...
namespace Asv.Gnss
{
    ...
    public class SbfPacketPvtGeodeticRev2:SbfPacketBase
    {
        ...
        {
            int offset = (int) (offsetBits / 8);
            Mode = buffer[offset];offset+=1;
            Solution = (SbfPVTSolution)(Mode & 0b0000_1111);
            Flag2D3D = (Mode >> 7) != 0;
            Error = buffer[offset];offset+=1;
            Latitude = CheckNan(BitConverter.ToDouble(buffer, offset));offset+=8;
            Longitude = CheckNan(BitConverter.ToDouble(buffer, offset)); offset += 8;
            Height = CheckNan(BitConverter.ToDouble(buffer, offset)); offset += 8;
            Undulation = CheckNan(BitConverter.ToSingle(buffer, offset)); offset += 4;
            Vn = CheckNan(BitConverter.ToSingle(buffer, offset)); offset += 4;
            Ve = CheckNan(BitConverter.ToSingle(buffer, offset)); offset += 4;
            Vu = CheckNan(BitConverter.ToSingle(buffer, offset)); offset += 4;
            COG = CheckNan(BitConverter.ToSingle(buffer, offset)); offset += 4;
            RxClkBias = CheckNan(BitConverter.ToDouble(buffer, offset)); offset += 8;
            RxClkDrift = CheckNan(BitConverter.ToSingle(buffer, offset)); offset += 4;
            TimeSystem = (SbfPVTSolutionTime) buffer[offset];offset++;
            Datum = (SbfDatum)buffer[offset]; offset++;
            NrSV = buffer[offset]; offset++;
            WACorrInfo = (SbfWACorrInfoEnum)buffer[offset]; offset++;
            ReferenceID = BitConverter.ToUInt16(buffer, offset); offset += 2;
            MeanCorrAge = BitConverter.ToUInt16(buffer, offset) * 0.01; offset += 2;
            SignalInfo = BitConverter.ToUInt32(buffer, offset); offset += 4;
            AlertFlag = buffer[offset];offset+=1;
            ...
            NrBases = buffer[offset]; offset += 1;
            PPPInfo = BitConverter.ToUInt16(buffer, offset); offset+=2;
            AgeOfLastSeed = PPPInfo & 0b0000_1111_1111_1111;
            TypeOdLastSeed = (SbfTypeOdLastSeedEnum)(PPPInfo >> 14);

            // REV 2
            Latency = BitConverter.ToUInt16(buffer, offset) * 0.0001; offset += 2;
            HAccuracy = BitConverter.ToUInt16(buffer, offset) * 0.01; offset += 2;
            VAccuracy = BitConverter.ToUInt16(buffer, offset) * 0.01; offset += 2;
            Misc = buffer[offset];
