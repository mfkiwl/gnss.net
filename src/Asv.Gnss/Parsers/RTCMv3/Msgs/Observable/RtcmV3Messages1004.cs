...
namespace Asv.Gnss
{
    ...
    public class RtcmV3Messages1004 : RtcmV3RTKObservableMessagesBase
    {
        ...
        public override uint Deserialize(byte[] buffer, uint offsetBits = 0)
        {
            ...
            for (var i = 0; i < SatelliteCount; i++)
            {
                ...
                bitIndex += satellite.Deserialize(buffer, bitIndex);
    ...
    public class GPSSatellite : ISerializable
    {
        ...
        public uint Deserialize(byte[] buffer, uint offsetBits)
        {
            ...
            SatelliteId = (byte)RtcmV3Helper.GetBitU(buffer, bitIndex, 6); bitIndex += 6;
            L1CodeIndicator = (byte)RtcmV3Helper.GetBitU(buffer, bitIndex, 1); bitIndex += 1;
            L1Pseudorange = RtcmV3Helper.GetBitU(buffer, bitIndex, 24); bitIndex += 24;
            L1PhaseRange = RtcmV3Helper.GetBits(buffer, bitIndex, 20); bitIndex += 20;
            L1LockTime = (byte)RtcmV3Helper.GetBitU(buffer, bitIndex, 7); bitIndex += 7;
            IntegerL1PseudorangeModulusAmbiguity = (byte)RtcmV3Helper.GetBitU(buffer, bitIndex, 8); bitIndex += 8;
            L1CNR = (byte)RtcmV3Helper.GetBitU(buffer, bitIndex, 8); bitIndex += 8;

            L2CodeIndicator = (byte)RtcmV3Helper.GetBitU(buffer, bitIndex, 2); bitIndex += 2;
            L2L1PseudorangeDifference = RtcmV3Helper.GetBits(buffer, bitIndex, 14); bitIndex += 14;
            L2PhaseRange = RtcmV3Helper.GetBits(buffer, bitIndex, 20); bitIndex += 20;
            L2LockTime = (byte)RtcmV3Helper.GetBitU(buffer, bitIndex, 7); bitIndex += 7;
            L2CNR = (byte)RtcmV3Helper.GetBitU(buffer, bitIndex, 8); bitIndex += 8;
